...
public class ScoreDAOImpl implements ScoreDAO {
	...
	public ScoreDTO findById(String hak) {
		...
		try {
			...
			// 프로시저는 excuteUpdate() 로 실행
			cstmt.executeQuery();
			
			// OUT 파라미터 넘겨 받기
			// 프로시저 SYS_REFCURSOR는 ResultSet으로 반환
	...
	public List<ScoreDTO> listScore(String name) {
	    ...
	    try {
	        sql = " CALL findByName(?,?) ";  // 수정: INSTR 함수로 이름 검색
