...
class Test4 {
	...
	// 인스턴스변수
	int a; // 메모리를 할당 받을때 0으로 초기화
	int b = 10; // 메모리를 할당 받을때 10으로 초기화
	
	// System.out.println("test"); // 컴파일오류. 선언부에서는 실행문이 올 수 없다.
	
	// 클래스 변수
	static int c = 100;
	
	// 인스턴스 메소드
	public int sub(int n) { // n:매개변수(지역변수의일종)
		int s = 0; // 지역변수
	...
	public static void print() {
		// 클래스 메소드는 객체를 생성하지 않고 인스턴스 변수나 인스턴스 메소드는 바로 호출 불가
		// 클래스 메소드는 this, super 키워드 사용 불가
		// System.out.println(a); // 컴파일 오류
		// disp(); // 컴파일 오류
		
		// 클래스 변수나 다른 클래스 메소드는 호출 가능
		// 동일한 클래스에 존재하는 클래스변수나 클래스메소드를 호출할 경우
		// 클래스명 생략 가능
		System.out.println(c);
		write();
		
		// 인스턴스변수, 인스턴스메소드는 객체를 생성해서 호출한다.
		Test4 obj = new Test4();
